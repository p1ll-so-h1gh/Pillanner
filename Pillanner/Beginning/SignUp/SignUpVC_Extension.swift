...
// 소셜 로그인 아이콘 세개 다 한번 씩 눌러서 링크나오는거 보여주고 -> 회원가입 페이지 가서 회원가입 -> 이니셜셋업 진행 하고(2개정도)
// -> 유저메인뷰에 약 추가된거 보여주고, 약 추가 버튼눌러서 하나 더 추가하고 -> 약 수정하기 들어가서 수정 하는거 보여주고 -> 캘린더 뷰에 셀들 만들어진거 보여주고, 반정도 눌러서 달성률 50프로정도 맞춰서 계산되는거 보여주고 -> 캘린더 뷰 셀 다 눌러서 폭죽 보여주고 
